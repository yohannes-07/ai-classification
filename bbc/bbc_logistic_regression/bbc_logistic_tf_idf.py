...
import numpy as np  # only for sigmoid function due to overflow
import math
import random
...
for i in range(len(matrix_data)):
    n = len(matrix_data[i])
    matrix_data[i]['idf'] = math.log(N/(1+n))
...
def vectorize(matrix_data, class_labels):
    ...
    for i in range(len(class_labels)):
        ...
        # print(vector)
        doc_words = sum(vector)
        tf_idf_vector = [(vector[k]/doc_words)*matrix_data[k]['idf'] for k in range(len(vector))]
        feature_vectors.append(tf_idf_vector)
